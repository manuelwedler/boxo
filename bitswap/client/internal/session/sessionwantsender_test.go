...
func (*mockPeerManager) ForwardWants(context.Context, []cid.Cid, []peer.ID) error { return nil }
...
func TestSendWants(t *testing.T) {
	...
	spm := newSessionWantSender(sid, pm, fpm, swc, bpm, onSend, onPeersExhausted, false)
	defer spm.Shutdown()
...
func TestPeersExhausted(t *testing.T) {
	...
	ep := exhaustedPeers{}
	spm := newSessionWantSender(sid, pm, fpm, swc, bpm, onSend, ep.onPeersExhausted, false)
